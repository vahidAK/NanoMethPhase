...
if [[ -n "$1" && -n "$2" && -n "$3" && -n "$4" ]]; then
	...
	awk -F'\t' '$1 ~ /Y/ && $4 != "." && $5 !=  "." && length($4)==1 && length($5)==1' $2 | awk -F'\t' '$10 ~ /\/1/ || $10 ~ /\|1/ || $10 ~ /^1/' | awk 'BEGIN{ FS=OFS="\t" } NR==FNR { seen[$1,$2,$5]; next } ($1,$2,$5) in seen { print $0 }' - $3 | awk -F'\t' '$4 != "." && $5 !=  "." && length($4)==1 && length($5)==1' | awk -F'\t' '$10 ~ /\/1/ || $10 ~ /\|1/ || $10 ~ /^1/' >> $4"_PternalSNVs.vcf" 



	awk 'BEGIN{ FS=OFS="\t" } NR==FNR { seen[$1,$2,$5]; next } !($1,$2,$5) in seen { print $0 }' $2 $1 | awk -F'\t' '$1 !~ /X/ && $1 !~ /Y/ && $4 != "." && $5 !=  "." && length($4)==1 && length($5)==1' | awk -F'\t' '$10 ~ /1\/1/ || $10 ~ /0\/1/ || $10 ~ /1\/0/ || $10 ~ /1\|1/ || $10 ~ /0\|1/ || $10 ~ /1\|0/' | awk 'BEGIN{ FS=OFS="\t" } NR==FNR { seen[$1,$2,$5]; next } ($1,$2,$5) in seen { print $0 }' - $3 | awk -F'\t' '$4 != "." && $5 !=  "." && length($4)==1 && length($5)==1' | awk -F'\t' '$10 ~ /0\/1/ || $10 ~ /1\/0/ || $10 ~ /0\|1/ || $10 ~ /1\|0/' > $4"_MaternalSNVs.vcf"
	...
	awk -F'\t' '$1 ~ /X/ && $4 != "." && $5 !=  "." && length($4)==1 && length($5)==1' $1 | awk -F'\t' '$10 ~ /\/1/ || $10 ~ /\|1/ || $10 ~ /^1/' | awk 'BEGIN{ FS=OFS="\t" } NR==FNR { seen[$1,$2,$5]; next } ($1,$2,$5) in seen { print $0 }' - $3 | awk -F'\t' '$4 != "." && $5 !=  "." && length($4)==1 && length($5)==1' | awk -F'\t' '$10 ~ /\/1/ || $10 ~ /\|1/ || $10 ~ /^1/' >> $4"_MaternalSNVs.vcf"
	...
	Example: Trio_To_PhaseVCF_4MaleChild.sh Maternal.vcf Paternal.vcf Child.vcf Output
